...
All scripts are present in https://github.com/openumea/openumea-deployment 

The backup script is generated by: 
https://github.com/openumea/openumea-deployment/blob/master/payload/70-install-db-backup-script.sh

A restore is performed by: 
https://github.com/openumea/openumea-deployment/blob/master/payload/60-restore-create-database.sh

I.e, the system takes backup when it is a running and a restore is a redeploy of the system with 
a backup-file as an input. 

That keeps the system configured in the same way without the need for tinkering in order to restore the system
